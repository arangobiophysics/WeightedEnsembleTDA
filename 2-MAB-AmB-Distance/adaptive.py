...
bintargetcount=2#number of walkers per bin
numberofdim=1  # number of dimensions
binsperdim=[2]   # You will have prod(binsperdim)+numberofdim*(2+2*splitIsolated)+activetarget bins total
pcoordlength=50 # length of the pcoord
PCA=False       # choose to do principal component analysis
maxcap=[30]	#these are in the order of the dimensions left is first dimension and right is second dimension
mincap=[0]
targetstate=[9.9]    #enter boundaries for target state or None if there is no target state in that dimension
